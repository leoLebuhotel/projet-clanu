function [parameters,costs] = model(database, layers_dims, num_iterations, learning_rate, print_cost)
    ...
    X_v = database.X_valid;
    Y_v = database.Y_valid;
    costs = [];
    ...
    for i=1:num_iterations
        ...
        [AL_v,~] = L_model_forward(X_v,parameters);

        %-- Cost function. Inputs: "AL, Y, parameters". Outputs: "cost".
        [cost] = compute_cost(AL, Y);
        [cost_v] = compute_cost(AL_v,Y_v);
        ...
        if ( print_cost && (mod(i,100)==0) )
            disp(['Cost after iteration ', num2str(i), ': ', num2str(cost(1))])
            pause(0.01);
        end
